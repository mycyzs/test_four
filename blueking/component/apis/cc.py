...
class CollectionsCC(object):
    ...
    def __init__(self, client):
        ...
        self.search_all_objects = ComponentAPI(
            client=self.client, method='POST', path='/api/c/compapi{bk_api_ver}/cc/search_objects/',
            description=u'查询模型分类下的所有模型',
        )

        self.search_inst = ComponentAPI(
            client=self.client, method='POST', path='/api/c/compapi{bk_api_ver}/cc/search_inst/',
            description=u'查询模型下所有实例',
        )